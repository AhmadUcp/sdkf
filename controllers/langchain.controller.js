...
const getOpenAIResponse = async (req, res) => {
  ...
  if (relevantDocuments?.length > 0) {
    const documentContext = relevantDocuments?.map(item => item.pageContent)?.join('.');
    ...
    const SYSTEM_TEMPLATE = `As an AI assistant you provide answers based on the given context, ensuring accuracy and brifness. 
          ...
          -Answer should be in html markup unordered list
    ...
    const response = await chatModel.invoke(SYSTEM_TEMPLATE);
